...
postgresql://{{$db_user}}:{{.Values.db_password | default (tuple . $db_user | include "postgres.password_for_user")}}@{{include "db_host" . }}:{{.Values.postgres.port_public}}/{{ default .Values.name .Values.db_name | default .Chart.Name }}
    {{- else }}
        {{- $envAll := index . 0 }}
        {{- $name := index . 1 }}
        {{- $user := index . 2 }}
